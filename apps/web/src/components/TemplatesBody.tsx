...
export function TemplatesBody({ ledgerId, onUse }: TemplatesBodyProps) {
  ...
  return (
    <div className="grid gap-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-sm text-muted-foreground">常用交易一键复用，选择后填充分录，可再调整</p>
        <button type="button" className={buttonVariants({ size: 'sm' })} onClick={() => setOpen(true)}>
          <Plus /> 新建模板
        </button>
      </div>

      {templates.loading && templates.data == null && <Skeleton className="h-32" />}
      {notImplemented && <NotImplemented feature="交易模板" />}
      {!templates.loading && !notImplemented && templates.error && (
        <p className="text-sm text-destructive">加载失败：{templates.error}</p>
      )}

      {templates.data &&
        (templates.data.length === 0 ? (
          <p className="py-8 text-center text-sm text-muted-foreground">还没有模板</p>
        ) : (
