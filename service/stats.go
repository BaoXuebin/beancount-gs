...
func MonthsList(c *gin.Context) {
	...
	queryParams.OrderBy = "year, month desc" // 固定排序规则
	...
	// 调试输出原始查询结果
	if script.IsDebugMode() {
		fmt.Printf("Raw YearMonthList from BQL:\n")
		for i, item := range yearMonthList {
			fmt.Printf("  [%d] Year: '%s', Month: '%s'\n", i, item.Year, item.Month)
		}
	}
	...
	for i, yearMonth := range yearMonthList {
		if script.IsDebugMode() {
			fmt.Printf("Processing item [%d]: Year='%s', Month='%s'\n", i, yearMonth.Year, yearMonth.Month)
		}
	...
	// 调试输出最终结果
	if script.IsDebugMode() {
		fmt.Printf("Final months array: %v\n", months)
...
func StatsTotal(c *gin.Context) {
	...
	ledgerConfig := script.GetLedgerConfigFromContext(c)
	if script.IsDebugMode() {
		script.LogInfo(ledgerConfig.Mail, "DEBUG: StatsTotal 函数开始执行")
	}

	// 获取查询参数
	queryParams := script.GetQueryParams(c)
	if script.IsDebugMode() {
		script.LogInfo(ledgerConfig.Mail, fmt.Sprintf("DEBUG: 查询参数: %+v", queryParams))
	}

	// 构建BQL查询语句 - 直接查询账户类型和金额
	selectBql := fmt.Sprintf("SELECT '\\', root(account, 1), '\\', sum(convert(value(position), '%s')), '\\'", ledgerConfig.OperatingCurrency)
	if script.IsDebugMode() {
		script.LogInfo(ledgerConfig.Mail, fmt.Sprintf("DEBUG: BQL 查询语句: %s", selectBql))
	}

	// 准备结果列表
	accountTypeTotalList := make([]StatsResult, 0)
	if script.IsDebugMode() {
		script.LogInfo(ledgerConfig.Mail, "DEBUG: 正在执行 BQL 查询...")
	}
	...
	if err != nil {
		if script.IsDebugMode() {
			script.LogInfo(ledgerConfig.Mail, fmt.Sprintf("DEBUG: BQL 查询失败，错误: %v", err))
		}
		InternalError(c, err.Error())
		return
	}
	...
	if script.IsDebugMode() {
		script.LogInfo(ledgerConfig.Mail, fmt.Sprintf("DEBUG: BQL 查询返回 %d 个结果", len(accountTypeTotalList)))
		for i, result := range accountTypeTotalList {
			script.LogInfo(ledgerConfig.Mail, fmt.Sprintf("DEBUG: 结果 #%d - 账户: %s, 金额: %s", i+1, result.Account, result.Amount))
		}
	}
	...
	// 遍历结果列表，提取账户类型和金额
	for _, item := range accountTypeTotalList {
		if script.IsDebugMode() {
			script.LogInfo(ledgerConfig.Mail, fmt.Sprintf("DEBUG: 正在处理项目 - 账户: %s, 金额: %s", item.Account, item.Amount))
		}

		// 检查账户类型是否为主要账户类型
		if item.Account == "Income" || item.Account == "Assets" || item.Account == "Liabilities" ||
			item.Account == "Expenses" || item.Account == "Equity" {

			// 使用统一的金额解析函数
			amount, currency, err := parseAmountAndCurrency(item.Amount)
			if err != nil {
				if script.IsDebugMode() {
					script.LogInfo(ledgerConfig.Mail, fmt.Sprintf("DEBUG: 无法解析金额: %s, 错误: %v", item.Amount, err))
				}
				continue
			}
			...
			if currency != ledgerConfig.OperatingCurrency {
				if script.IsDebugMode() {
					script.LogInfo(ledgerConfig.Mail, fmt.Sprintf("DEBUG: 货币不匹配：预期 %s, 实际 %s", ledgerConfig.OperatingCurrency, currency))
				}
			}

			if script.IsDebugMode() {
				script.LogInfo(ledgerConfig.Mail, fmt.Sprintf("DEBUG: 从原始字符串 %s 中提取的金额: %s", item.Amount, amount.String()))
			}

			result[item.Account] = amount.Round(2).String()
		} else {
			if script.IsDebugMode() {
				script.LogInfo(ledgerConfig.Mail, fmt.Sprintf("DEBUG: 跳过非根账户类型: %s", item.Account))
			}
		}
	}

	if script.IsDebugMode() {
		script.LogInfo(ledgerConfig.Mail, fmt.Sprintf("DEBUG: 最终结果包含 %d 个条目", len(result)))
		for key, value := range result {
			script.LogInfo(ledgerConfig.Mail, fmt.Sprintf("DEBUG: 结果条目 - 键: %s, 值: %s", key, value))
		}
		script.LogInfo(ledgerConfig.Mail, "DEBUG: StatsTotal 函数执行完成")
	}
...
func StatsAccountTrend(c *gin.Context) {
	...
	for _, stats := range statsResultList {
		...
		*/
		...
		if err != nil {
			log.Printf("Warning: 无法解析金额: %s, error: %v", selectedCommodity, err)
			continue
		}
...
func StatsMonthCalendar(c *gin.Context) {
	...
	// 构建BQL查询语句
	bql := fmt.Sprintf(
		"SELECT '\\', date, '\\', root(account, 1), '\\', sum(convert(value(position), '%s')), '\\'",
		ledgerConfig.OperatingCurrency,
	)
